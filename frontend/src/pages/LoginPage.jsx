...
export default function LoginPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(e.response?.data?.detail || "Đăng nhập thất bại. Vui lòng kiểm tra lại.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen w-full flex items-center justify-center bg-slate-100 p-4">
            ...
            <div className="w-full max-w-md bg-white border border-slate-200/60 rounded-3xl shadow-xl p-10">
                ...
                {/* Thông báo lỗi */}
                {error && (
                    <div className="mb-6 p-4 rounded-xl bg-red-50 text-red-600 text-sm border border-red-100 flex items-start gap-2.5">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className="w-full mt-2 bg-slate-900 text-white py-3.5 px-4 rounded-xl font-semibold hover:bg-slate-800 active:scale-[0.99] disabled:opacity-50 transition-all duration-150 flex items-center justify-center gap-2"
                    >
                        {isLoading ? (
